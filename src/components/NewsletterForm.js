...
const NewsletterForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!active) {
      ...
      // to gsap animation - animation for plane
      to(button, {
  ...
  return (
    <div className="flex flex-col space-y-8 md:w-[400px]">
      <form
        ...
        className="newsletter-form mt-10 duration-1000 animate-fade-in-3"
      >
        <div className="group flex items-center gap-x-4 py-1 pl-4 pr-1 rounded-[9x] bg-[#999999] hover:bg-[#777777] shadow-outline-gray hover:shadow-transparent focus-within:bg-[#555555] focus-within:!shadow-outline-gray-focus transition-all duration-300">
          <EnvelopeIcon className="hidden sm:inline w-g h-6 text-[#4B4C52] group-focus-within:text-white group-hover:text-white transition-colors duration-300"/>
          <input
            value={input}
          ...
          <button ref={buttonRef} className={`${
            active && "active"
          } disabled:!bg-[#17141F] disabled:grayscale-[65%] disabled:opacity-50 disabled:cursor-not-allowed text-sm md:text-base`} disabled={!input} type="submit">
      ...
      <div className="relative">
        {(successMessage || errorMessage) && (
          <div className="flex items-start space-x-2 bg-[#0A0E12] shadow-outline-gray text-white rounded-[9px] py-4 px-6 animate-fade-bottom absolute">
            <div className="h-6 w-6 bg-[#1B2926] flex items-center justify-center rouded-full border border-[#273130] flex-shrink-0">
